...
// Function to download image using direct HTTP request with proper headers
function downloadImageDirectHttp(imageUrl, filepath) {
...
// Function to extract product images with multiple fallback methods
async function extractProductImages(page, productUrl) {
  ...
  const imageData = await page.evaluate(() => {
    ...
    // Method 3: Check for data attributes with high quality images
    ...
    // Method 4: Check for product image containers
    ...
    // Method 5: Fallback - find any Bulgari images
    if (images.length === 0) {
...
async function downloadAllBvlgariImagesFinal() {
  console.log('🔍 Starting Bvlgari Le Gemme perfume image download (Final attempt)...');
      ...
      try {
        ...
        for (let j = 0; j < Math.min(imageUrls.length, product.images.length); j++) {
          ...
          try {
            ...
            // Try to download with direct HTTP request first
            try {
              await downloadImageDirectHttp(imageUrl, filepath);
              const stats = fs.statSync(filepath);
              ...
              // If direct HTTP fails, try with Puppeteer
              console.log(`   ⚠️  Direct download failed, trying with Puppeteer...`);
...
// Run the final download
downloadAllBvlgariImagesFinal().catch(console.error);