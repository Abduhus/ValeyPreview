...
export default function ProductCard({ product, isRecommended = false, currency = "AED", exchangeRate = 1 }: ProductCardProps) {
  ...
  if (product.moodImageUrl) uniqueImages.add(product.moodImageUrl);
    ...
    >
      ...
      <div className="relative z-20">
        ...
        <div className="relative w-full h-96 overflow-hidden">
          ...
          <img 
            src={highQualityImages[currentImageIndex] || product.imageUrl} 
            alt={product.name} 
      ...
      <div className="p-8 relative z-20">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-1" data-testid={`rating-product-${product.id}`}>
            ...
            <span className="text-xs text-muted-foreground ml-1 group-hover:text-foreground transition-colors duration-300">({product.rating})</span>
          </div>
          <div className="text-xs text-muted-foreground uppercase tracking-wider group-hover:text-foreground transition-colors duration-300">
            SKU: PU{product.id.toString().padStart(4, '0')}
          </div>
        </div>
          ...
          <div className="mb-6 p-4 bg-gradient-to-r from-primary/8 via-primary/5 to-accent/8 border border-primary/25 rounded-lg backdrop-blur-sm">
            ...
            <div className="grid grid-cols-3 gap-3 text-xs">
              <div className="text-center bg-card/40 rounded-md p-2">
                ...
                <div className="text-muted-foreground">{formatPrice(product.price)}</div>
              </div>
              <div className="text-center bg-card/40 rounded-md p-2">
                <div className="font-semibold text-foreground">6-20 pcs</div>
                <div className="text-green-600 font-semibold">{formatPrice((parseFloat(product.price) * 0.95).toFixed(2))}</div>
              </div>
              <div className="text-center bg-card/40 rounded-md p-2">
                <div className="font-semibold text-foreground">21+ pcs</div>
                <div className="text-green-600 font-semibold">{formatPrice((parseFloat(product.price) * 0.90).toFixed(2))}</div>
              </div>
            </div>
          </div>
        )}
